...
const ServiceItem = ({nameService=null, key_id=null, children, ...props}) => {
    ...
    const showCard = () => {
        gsap.to(`div#${key_id}`, {duration: .6, scale: 1, opacity: 1, ease: "expoScale(1, 5)"});
    }

    const hideCard = () => {
        gsap.to(`div#${key_id}`, {duration: .6, scale: 0.5, opacity: 0, ease: "expoScale(1, 5)"});
    }
  ...
  return (
    <div id={`show_${key_id}`} className='hoverServices titleServicesContent' style={{ zIndex: 100-(1+props.idx) }} >
        ...
        <div className='contentCard' >
            <div id={`card_${key_id}`} className='cardServices' >
                {children}
            </div>
        </div>
    </div>
